...
# In 'run_worker'
    ...
    device = torch.device('cpu',2 * rank)
...
# In 'run_worker'
    ...
    tmp_list = [Encoder(ntokens, emsize, dropout).to(torch.device('cpu', 2 * rank))] #.cuda(2 * rank)]
    module_list = []
    ...
    for i in range(nlayers):
        ...
        tmp_list.append(transformer_block.to(torch.device('cpu', 2 * rank + device)))

    # Add decoder in the end.
    tmp_list.append(Decoder(ntokens, emsize).to(torch.device('cpu', 2 * rank + num_gpus - 1))) #.cuda(2 * rank + num_gpus - 1))
    module_list.append(nn.Sequential(*tmp_list))
...
# In 'run_worker'
    ...
    def train():
        ...
        for batch, i in enumerate(range(0, nbatches, bptt)):
            ...
            # pipeline resides.
            loss = criterion(output.view(-1, ntokens), targets.to(torch.device('cpu', 2 * rank + 1))) #.cuda(2 * rank + 1))
    ...
    def evaluate(eval_model, data_source):
        ...
        with torch.no_grad():
            for i in range(0, nbatches, bptt):
                ...
                # Need to move targets to the device where the output of the
                # pipeline resides.
                total_loss += len(data) * criterion(output_flat, targets.to(torch.device('cpu', 2 * rank + 1))).item() #.cuda(2 * rank + 1)).item()
